...
@section('content')
    ...
    @else
        @php
            $categoryTitles = [
                'bimbel-ukom' => 'Bimbel UKOM D3 Farmasi',
                'cpns-p3k' => 'CPNS & P3K Farmasi',
                'joki-tugas' => 'Joki Tugas Farmasi'
            ];
        @endphp

        <!-- Programs grouped by class category -->
        @foreach($groupedPrograms as $category => $programs)
            <div class="mb-8">
                <h2 class="text-xl font-bold text-gray-900 mb-4">{{ $categoryTitles[$category] ?? $category }}</h2>
