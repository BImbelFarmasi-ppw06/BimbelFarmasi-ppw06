                        ...
                        <div class="bg-gradient-to-r from-[#2D3C8C] to-[#1e2761] px-6 py-4">
                            <div class="flex flex-wrap items-center justify-between gap-3">
                                ...
                                @if($order->payment)
                                    ...
                                    @endif
                                @else
                                    <span class="px-4 py-2 bg-orange-500 text-white rounded-full text-sm font-semibold animate-pulse">
                                        💳 Belum Bayar
                                    </span>
                                @endif
                            </div>
                            ...
                            <div class="flex flex-wrap items-center justify-between gap-4 pt-4 border-t">
                                ...
                                <div class="flex flex-wrap gap-3">
                                    ...
                                    @elseif($order->payment && $order->payment->status === 'pending')
                                        <!-- Pembayaran pending - bisa lanjutkan atau batalkan -->
                                        <a href="{{ route('order.payment', $order->order_number) }}" class="inline-flex items-center gap-2 bg-green-600 text-white font-semibold px-6 py-3 rounded-lg hover:bg-green-700 transition shadow-lg">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/>
                                            </svg>
                                    ...
                                    @if($order->payment && $order->payment->status === 'paid')
                                        <!-- Testimoni & Kontak hanya untuk order yang sudah lunas -->
                                        @php
...
function cancelPayment(orderNumber) {
    if (!confirm('Apakah Anda yakin ingin membatalkan pembayaran ini? Anda dapat membayar kembali nanti jika berubah pikiran.')) {
        return;
    }
